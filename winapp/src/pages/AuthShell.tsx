...
/**
 * Каркас экранов входа/регистрации — перенесён из веб-клиента ради дизайн-
 * паритета (постер-аside слева, карточка формы справа). Без ссылок на гостевой
 * вход: в WinApp его нет.
 */
    ...
    >
      ...
      <main style={{ display: 'grid', placeItems: 'center', padding: isMobile ? '24px 16px' : 32 }}>
        <div style={surfaceStyle}>
          ...
          {children}
          <div style={{ marginTop: 4, color: 'var(--text-2)', fontSize: 13, textAlign: 'center' }}>{footer}</div>
        </div>
      </main>
    </div>
  );
...
export function Field({ label, type = 'text', value, onChange, placeholder, autoComplete, minLength, name }: FieldProps) {
  return (
