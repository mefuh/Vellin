...
export interface UserC2SDmSend {
  ...
  /**
   * Клиент уже применил финальную ориентацию (зеркало фронт-камеры / нормальная
   * задняя) при записи через canvas-конвейер со сменой камеры — сервер НЕ должен
   * делать hflip. Если отсутствует/false — сырьё снято прямым потоком с фронталки
