...
export interface DirectMessageDTO {
  ...
  voicePlayed?: boolean;
  /**
   * Видеосообщение («кружок»): готовый mp4 (`/api/uploads/dm-video/...`). Пока
   * идёт транскод — отсутствует, а videoStatus='processing'.
   */
  videoUrl?: string;
  /** Постер-кадр видеосообщения (jpg) — первый кадр для бабла до воспроизведения. */
